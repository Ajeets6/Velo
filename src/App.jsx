...
function GuidePanel({ session, onAction, onStartOver }) {
  ...
  return <section className="guide-panel" aria-live="polite"><div className="guide-progress"><span>Guide progress</span><strong>{Math.round(session.progress * 100)}%</strong><div><i style={{ width: `${session.progress * 100}%` }} /></div></div><h1>{session.goal}</h1><p className="guide-question">{session.message?.nextQuestion || session.currentQuestion}</p>{session.message?.feedback && <p className="guide-feedback">{session.message.feedback}</p>}{session.message?.hint && <p className="guide-hint">Hint: {session.message.hint}</p>}<div className="guide-actions"><button onClick={() => onAction("hint")}>Hint</button><button onClick={() => onAction("explain")}>Explain this step</button><button onClick={() => onAction("skip")}>Skip</button><button onClick={() => onAction("visual")}>Show a visual</button><button onClick={onStartOver}>Start over</button></div>{session.history.length > 0 && <details className="guide-history"><summary>Previous steps ({session.history.length})</summary>{session.history.map((item, index) => <p key={`${item.at}-${index}`}><strong>{item.question}</strong><br />{item.feedback}</p>)}</details>}</section>;
}
...
function Tutor({ onBack, theme, onToggleTheme }) {
  ...
  async function startGuide(question) {
    const response = await fetch("/api/guide/sessions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ prompt: question, learnerLevel }) });
    const session = await response.json(); if (!response.ok) throw new Error(session.error?.message || "Velo could not start this guide."); setGuideSession(session); setPrompt("");
  }
  async function guideAction(action) {
    if (!guideSession) return;
    try { const response = await fetch(`/api/guide/sessions/${guideSession.id}/messages`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, answer: action === "answer" ? prompt.trim() : "" }) }); const session = await response.json(); if (!response.ok) throw new Error(session.error?.message || "The guide could not continue."); setGuideSession(session); if (action === "visual") { setMode("visualize"); setPrompt(session.message?.visualPrompt || guideSession.prompt); } else if (action === "answer") setPrompt(""); } catch (error) { setResult({ title: "Guide interrupted", answer: error.message, nextStep: "Try again in a moment." }); }
  }
  async function startGuideOver() { if (!guideSession) return; await fetch(`/api/guide/sessions/${guideSession.id}`, { method: "DELETE" }); setGuideSession(null); }
  function speak() {
  ...
  return <main className="tutor-screen"><header className="tutor-header"><button className="icon-button" onClick={onBack} aria-label="Back to welcome"><ArrowLeft weight="bold" /></button><a className="brand compact" href="#" onClick={(event) => { event.preventDefault(); onBack(); }}><span className="brand-icon"><Atom weight="duotone" /></span><span>Velo</span></a><div className="header-actions"><ThemeToggle theme={theme} onToggle={onToggleTheme} /><div className={`backend-state ${backendOnline ? "online" : "offline"}`}><span />{backendOnline ? "Local backend" : "Reconnecting"}</div></div></header><div className="tutor-body"><section className="mode-section" aria-labelledby="mode-label"><p id="mode-label">How would you like to learn?</p><div className="mode-switcher">{modes.map(({ id, label, icon: Icon }) => <button key={id} className={mode === id ? "active" : ""} onClick={() => setMode(id)} aria-pressed={mode === id}><Icon weight={mode === id ? "fill" : "regular"} />{label}</button>)}</div></section><section className="voice-section"><button className={`voice-button ${speaking ? "speaking" : ""}`} onClick={speak} aria-label={speaking ? "Stop speaking" : "Read response aloud"}>{speaking ? <Pause weight="fill" /> : <SpeakerHigh weight="fill" />}</button><span>{speaking ? "Speaking" : "Listen"}</span></section><section className={`output-card ${loading ? "loading" : ""}`} aria-live="polite" aria-busy={loading}><div className="output-label"><Sparkle weight="fill" /> VELO</div>{loading ? <div className="thinking"><span /><span /><span /><p>Thinking through your question…</p></div> : <div className="output-copy"><h1>{result.title}</h1><p>{result.answer}</p>{result.nextStep && <div className="next-step"><CheckCircle weight="fill" /><span>{result.nextStep}</span></div>}{result.motionforge && <div className="motionforge-note"><Waveform weight="duotone" /><span>MotionForge scene prepared</span></div>}</div>}</section>
    ...
    {mode === "guide" && guideSession && <GuidePanel session={guideSession} onAction={guideAction} onStartOver={startGuideOver} />}
