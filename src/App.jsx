...
function Tutor({ onBack, theme, onToggleTheme }) {
  ...
  async function cancelAnimation() {
    if (!animation?.id || !["queued", "running"].includes(animation.status)) return;
    try {
      const response = await fetch(`/api/animations/${animation.id}`, { method: "DELETE" });
      const job = await response.json();
      if (!response.ok) throw new Error(job.error?.message || "The animation could not be cancelled.");
      setAnimation(job);
    } catch (error) {
      setAnimation((current) => ({ ...current, status: "failed", stage: "Animation failed", error: error.message }));
    }
  }
      ...
      <div className="tutor-body">
        ...
        {animation && (
          <section className={`animation-card ${animation.status}`} aria-live="polite">
            ...
            ) : ["failed", "cancelled"].includes(animation.status) ? (
              <div className="animation-status error"><WarningCircle weight="duotone" /><div><strong>{animation.status === "cancelled" ? "Animation cancelled" : "Couldn’t create the animation"}</strong><span>{animation.error?.message || animation.error || "MotionForge stopped unexpectedly."}</span></div></div>
            ) : (
              <div className="animation-status"><SpinnerGap className="spin" weight="bold" /><div><strong>{animation.stage || "Generating animation…"}</strong><span>{animation.queuePosition ? `Queue position: ${animation.queuePosition}. ` : ""}Cloud Ollama is designing the scene, then MotionForge will simulate and render it.</span><button type="button" className="animation-cancel" onClick={cancelAnimation}>Cancel</button></div></div>
            )}
          </section>
        )}
